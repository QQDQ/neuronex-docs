...
- **网络与可达性**：导入 Neuron 后，Neuron 需能访问原采集所指向的**现场设备**（IP、端口、串口等以您在 Neuron 中最终配置为准），请确保网络与安全策略与迁移前计划一致。  
- **不支持的项会被跳过**：不支持的协议、超限或不兼容的地址/数据类型，可能无法进入最终 JSON，请在工具界面与日志/汇总中核对失败项，并在 Neuron 中**手工补配**或调整源侧后重新转换。  
- **以分文档为准**：工具能力与协议支持会随版本迭代；协议列表、限制与操作截图以**各分文档**中最新内容为准。  

如您在迁移过程中需扩展支持或遇到问题，可结合分文档中的说明与 Neuron 官方支持渠道进一步排查。
